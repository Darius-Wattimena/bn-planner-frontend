...
const DiscussionItemResponse = ({ response, users, userId, query }) => {
  ...
  const authorDetails = getUserWithId(users, response.authorOsuId)
  const authorRole = getReadableRole(authorDetails.role)
  const submitIsActive = /\S/.test(formValues.content)

  const formattedText = formatOsuLinks(response.content)
