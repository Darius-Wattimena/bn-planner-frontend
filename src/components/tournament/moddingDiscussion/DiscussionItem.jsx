...
const DiscussionItem = ({ discussion, users, userId, query }) => {
  const moddingComment = discussion.moddingComment
  ...
  const authorDetails = getUserWithId(users, moddingComment.authorOsuId)
  const authorRole = getReadableRole(authorDetails.role)
  const submitIsActive = timestampRegex.test(formValues.content)

  const formattedText = formatOsuLinks(moddingComment.content)
